...
export function handleAgentRecordingEvent(
  ...
  if (outerTypeDiag !== 'pw_screenshot') {
    console.log(`[RecorderWS-DIAG] handleAgentRecordingEvent — sid=${sid} found=${!!session} status=${session?.status} type=${outerTypeDiag} eventType=${eventData.eventType} uiClients=${session?.uiClients?.size ?? 0}`);
  }
  if (!session) return;
