...
test('Recorded flow', async ({ page, context }) => {
  ...
  const L = {
    ...
    paymentPlan         : page.locator('xpath=//*[@id=\'divEnableFeePayment\']//h2').filter({ visible: true }).first(),
    //  ↳ [relative-structural] xpath: //*[@id='divEnableFeePayment']//h2
  ...
  await smartFill(L.passwordInput, 'Mahim123');
  ...
  await smartFill(L.formnameInput, 'testing2 ');
  await smartCheck(L.feeenabledCheckbox);
  await smartFill(L.amountInput, '100');
  await smartFill(L.pertansactionfeeInput, '10');
  await smartClick(L.paymentPlan);
  await smartCheck(L.paymentplanenabledCheckbox);
  await smartClick(L.saveButton);
  await page.waitForURL('**https://ap-forms.rediker.com/FormsManager/CreateEditPages?formId=1359&formTypeId=1');
});