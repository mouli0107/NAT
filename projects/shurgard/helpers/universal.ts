...
// ── Smart helpers used by page classes ────────────────────────────────────────

/** Click a locator, waiting for visibility first.
 *  Uses playwright config's actionTimeout by default (pass explicit timeout to override). */
export async function smartClick(locator: Locator, timeout?: number): Promise<void> {
  if (timeout !== undefined) {
    await locator.waitFor({ state: 'visible', timeout });
  } else {
    await locator.waitFor({ state: 'visible' }); // uses actionTimeout from config
  }
  await locator.click();
}

/** Fill a locator, waiting for visibility first */
export async function smartFill(locator: Locator, value: string, timeout?: number): Promise<void> {
  if (timeout !== undefined) {
    await locator.waitFor({ state: 'visible', timeout });
  } else {
...
/** Check a checkbox/radio, waiting for visibility first */
export async function smartCheck(locator: Locator, timeout?: number): Promise<void> {
  if (timeout !== undefined) {
    await locator.waitFor({ state: 'visible', timeout });
  } else {
    await locator.waitFor({ state: 'visible' });
...
/** Uncheck a checkbox, waiting for visibility first */
export async function smartUncheck(locator: Locator, timeout?: number): Promise<void> {
  if (timeout !== undefined) {
    await locator.waitFor({ state: 'visible', timeout });
  } else {
    await locator.waitFor({ state: 'visible' });
