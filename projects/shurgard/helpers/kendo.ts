...
// Extend Window to include jQuery and Kendo (injected by the Shurgard app)
declare global {
  interface Window {
    jQuery: (selector: any) => any;
    kendo:  { parseDate: (value: string, formats: string | string[]) => Date | null };
  }
}
...
export async function selectKendoDropdown(page: Page, inputId: string, optionText: string): Promise<void> {
  ...
  var result = await page.evaluate(function(params) {
    try {
      ...
      if (!found) { w.ul.find('li').each(function(this: HTMLElement, idx: number) { if ($(this).text().trim() === params.optionText) { w.select(idx); w.trigger('change'); found = true; return false; } }); }
      if (!found) { w.ul.find('li').each(function(this: HTMLElement, idx: number) { if ($(this).text().trim().toLowerCase().indexOf(params.optionText.toLowerCase()) > -1) { w.select(idx); w.trigger('change'); found = true; return false; } }); }
